...
const Contact = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          let displayLocation = t('myCurrentLocation');
          if (data.address) {
            ...
            if (addressParts.length > 0) {
              ...
              displayLocation = `${t('location')}: ${latitude.toFixed(4)}, ${longitude.toFixed(4)} | ${googleMapsShareUrl}`;
            }
          } else {
            displayLocation = `${t('location')}: ${latitude.toFixed(4)}, ${longitude.toFixed(4)} | ${googleMapsShareUrl}`;
          }
  ...
  const handleSubmit = async (e) => {
    ...
    if (missingFields.length > 0) {
      setState(prev => ({
        ...
        status: {
          type: 'error',
          message: `${t('fillAllFields')} ${missingFields.map(field => t(field)).join(', ')}`
        }
      }));
      return;
    }
    ...
    setState(prev => ({
      ...prev,
      isLoading: true,
      status: { type: 'loading', message: t('schedulingService') }
    }));
    ...
    try {
      ...
      if (response && response.success !== false) {
        ...
        throw new Error(response.message || t('requiredFieldsMissing'));
      }
      ...
      if (error.message.includes('Required fields') || error.message.includes(t('requiredFieldsMissing'))) {
        errorMessage = t('requiredFieldsMissing');
      }
  ...
  const contactInfo = [
    ...
    },
    {
      icon: Mail,
      title: t('emailUs'),
      details: [t('email1'), t('email2'), t('email3')],
      color: '#d97706'
    },
    {
      icon: Clock,
          ...
          <div className="grid lg:grid-cols-3 gap-8 mb-16">
            ...
            >
              <div className="backdrop-blur-xl bg-gradient-to-br from-amber-900/20 to-orange-900/20 border-2 border-amber-500/30 rounded-2xl p-8 hover:border-amber-400/50 transition-all">
                ...
                <p className="text-amber-200/80 mb-6">{t('formSubheading')}</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div>
                      <label className="block text-amber-200 font-medium mb-2">{t('location')}</label>
                      <div className="relative">
                        ...
                        >
                          ...
                          {state.isGettingLocation ? t('detecting') : t('getLocation')}
                        </button>
                      </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      ...
                      >
                        <option value="" className="bg-amber-900">{t('selectVehicleType')}</option>
                        <option value="sedan" className="bg-amber-900">{t('sedan')}</option>
                        <option value="suv" className="bg-amber-900">{t('suv')}</option>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={t('carTroublePlaceholder')}
                    ></textarea>
                  </div>
            ...
            <div id="info" data-animate className={`space-y-6 transition-all duration-1000 delay-300 ${state.isVisible.info ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
              ...
              <div className="backdrop-blur-xl bg-gradient-to-br from-red-900/20 to-orange-900/20 border-2 border-red-500/30 rounded-2xl p-6 text-center group hover:bg-red-900/30 transition-all duration-500">
                <div className="flex items-center justify-center gap-3 mb-4">
                  ...
                  <h3 className="text-xl font-bold text-red-100">{t('emergencyService')}</h3>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href="tel:+96879265646">
                <button className="inline-block px-10 py-5 font-black rounded-xl text-xl shadow-2xl transition-all hover:scale-110 relative overflow-hidden group bg-gradient-to-r from-amber-500 to-orange-500 text-white">
                  <span className="relative z-10">{t('callNowForService')}</span>
                  <div className="absolute inset-0 bg-white/20 translate-y-full group-hover:translate-y-0 transition-transform"></div>
                </button>
              </a>
              <button className="px-10 py-5 font-black rounded-xl text-xl shadow-2xl transition-all hover:scale-110 relative overflow-hidden group backdrop-blur-xl bg-amber-900/20 border-2 border-amber-400/50">
                <span className="relative z-10 text-amber-100" onClick={() => navigate("/service")}>
                  {t('viewOurServices')}
                </span>
